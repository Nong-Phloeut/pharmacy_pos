  ...
  <v-container>
    <v-row class="mb-4">
      <v-col cols="12" class="d-flex justify-space-between align-center">
        <h1 class="text-h4 font-weight-bold">Admins</h1>
    ...
    <v-dialog v-model="dialog" max-width="500px">
      <v-card>
        <v-card-title class="text-h5">{{ formTitle }} Admin</v-card-title>
        <v-card-text>
...
const saveItem = () => {
  if (editedIndex.value > -1) {
    Object.assign(admins.value[editedIndex.value], editedItem.value);
  } else {
    editedItem.value.id = admins.value.length + 1;
    admins.value.push(editedItem.value);
  }
  closeDialog();
};

const deleteItem = (item) => {
  const index = admins.value.indexOf(item);
